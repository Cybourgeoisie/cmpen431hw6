...
il1 = ["il1:%s:%s:%s:l" %(x,(y*8),z) for x in [256,512] for y in [4,8] for z in [2]]

# After reviewing initial L2 results
#ul2 = ["ul2:%s:%s:%s:l" %(x,(y*8),z) for x in [2048,4096,8192] for y in [8,16] for z in [2,4]]
ul2 = ["ul2:%s:%s:%s:l" %(x,(y*8),z) for x in [2048,4096] for y in [16] for z in [2,4]]

makeConfigs("dyn2_int_opt_processor_params",
	{
...
# Run tests
#runBenchmarksOnTestSets(["dyn2_int_opt_processor_params"])

# Retrieve results
#extractDataFromResults(["sim_IPC"], ["dyn2_int_opt_processor_params"])
